...
class FileRow(BoxLayout):
    ...
    def __init__(self, path, name, **kwargs):
        ...
        self.height = '48dp'
        self.checkbox = CheckBox(size_hint_x=None, width='48dp')
        self.add_widget(self.checkbox)
        self.add_widget(Label(text=name, halign='left', valign='middle'))
        btn = Button(text='Info', size_hint_x=None, width='90dp')
        btn.bind(on_release=self.show_info)
        self.add_widget(btn)
...
class Box(BoxLayout):
    so_files = ListProperty([])
    current_path = StringProperty(os.path.expanduser('~'))
    status_text = StringProperty('Ready')
    selected_info = StringProperty('Select a file and press Info to see metadata')

    def browse(self):
        if filechooser:
            filechooser.choose_dir(on_selection=self._on_dir)
        else:
            self.status_text = 'filechooser not available. Enter path manually.'
    ...
    def scan_so_files(self):
        path = self.ids.path_input.text.strip() or self.current_path
